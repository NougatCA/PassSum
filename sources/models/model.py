...
from transformers.generation_stopping_criteria import validate_stopping_criteria, StoppingCriteriaList
from transformers.generation_logits_process import LogitsProcessorList

import logging
import warnings
from typing import Dict, Any, Union, Optional, Iterable, List, Callable
...
class PassSum(BartPretrainedModel):
        ...
        if "encoder_outputs" not in model_kwargs:
            ...
            encoder_kwargs = {
                ...
                if not (argument.startswith("decoder_") or argument.startswith("cross_attn")
                        or argument in ['code_input_ids', 'id_inputs', 'id_seq_lens', 'node_inputs',
                                        'node_seq_lens', 'path_seq_lens', 'labels'])
            }
            ...
            # model_kwargs['inputs_embeds'] = inputs_embedded
            model_kwargs["encoder_outputs"]: ModelOutput = encoder(inputs_embeds=inputs_embedded,
                                                                   return_dict=True,
                                                                   **encoder_kwargs)
        return model_kwargs
    ...
    def forward(
        ...
        if not encoder_outputs:
            ...
            # [B, T_path, H]
            path_embedded = self.path_encoding(id_inputs, id_seq_lens, node_inputs, node_seq_lens, path_seq_lens)
            inputs_embeds = torch.cat([code_embedded, path_embedded], dim=-2)     # [B, T_code+T_path, H]
    ...
    def generate(
        ...
        if self.config.is_encoder_decoder:
            ...
            model_kwargs = self._prepare_encoder_decoder_kwargs_for_generation(
                code_input_ids=model_kwargs['code_input_ids'],
                id_inputs=model_kwargs['id_inputs'],
                id_seq_lens=model_kwargs['id_seq_lens'],
                node_inputs=model_kwargs['node_inputs'],
                node_seq_lens=model_kwargs['node_seq_lens'],
                path_seq_lens=model_kwargs['path_seq_lens'],
